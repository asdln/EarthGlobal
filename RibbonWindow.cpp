...
{
	m_strResPath = QApplication::applicationFilePath();
	m_strResPath = QFileInfo(m_strResPath).absolutePath();
	m_strResPath += "/res/";

    RibbonToolTip::setWrapMode(RibbonToolTip::NativeWrap);
    createOptions();

	setStyleTheme(Qtitan::RibbonStyle::Office2016Colorful);

	m_actionRibbonMinimize = ribbonBar()->addAction(QIcon(m_strResPath + "ribbonMinimize.png"), tr("Minimize the Ribbon"), Qt::ToolButtonIconOnly);
...
void RibbonWindow::minimizationChanged(bool minimized)
{
	...
	QString strTest = m_strResPath + "ribbonMaximize.png";
	m_actionRibbonMinimize->setIcon(minimized ? QIcon(m_strResPath + "ribbonMaximize.png") :
		QIcon(m_strResPath + "ribbonMinimize.png"));
}
